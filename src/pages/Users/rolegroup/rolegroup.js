import {
  ...
  import CreateForm from './components/CreateRoleGroup';
  ...
  class RoleGroups extends Component {
    state = {
			modalVisible: false,
			current: null,
    ...
    handleSelectRows = rows => {
      this.setState({
		...
		deleteById = (id) => {
			const { dispatch } = this.props;
			dispatch({
				type: 'rolegroup/deleteRoleGroup',
				payload: {
					id: id
				},
				callback: () => {
		...
		deleteByIds = () => {
			...
			dispatch({
				type: 'rolegroup/deleteRoleGroups',
				payload: {
					ids: deleteid
				},
				callback: () => {
		...
		buttonEdit = (rowitem) => {
			...
			//先从服务器更新最新数据
			dispatch({
				type: 'rolegroup/detailRoleGroup',
				payload: {
					id: rowitem.id
				},
				callback: (item) => {
    ...
    handleUpdate = fields => {
      ...
      dispatch({
        type: 'rolegroup/updateRoleGroup',
				payload: fields,
				callback: {
					callbacksuccess: () => {
      ...
      return (
        <Form onSubmit={this.handleSearch} layout="inline">
          ...
          >
              ...
              <FormItem label="名称">
                {getFieldDecorator('name')(<Input placeholder="请输入角色组名称" />)}
              </FormItem>
            </Col>
            <Col sm={24} md={12} lg={8} xl={6} >
  ...
  export default Form.create()(RoleGroups);
