  ...
  import CreateForm from './components/CreatePermit';
  ...
  class PermitsList extends Component {
    ...
    renderSimpleForm() {
      const { form: {getFieldDecorator}, permits: {permitgroup} } = this.props;
      return (
            ...
            <Col sm={24} md={12} lg={8} xl={6} >
              <FormItem label="权限名称">
                {getFieldDecorator('name')(<Input placeholder="请输入权限名称" />)}
						...
						<Col sm={24} md={12} lg={8} xl={6} >
              <FormItem label="权限编码">
                {getFieldDecorator('code')(<Input placeholder="请输入权限编码" />)}
              </FormItem>
            </Col>
            <Col sm={24} md={12} lg={8} xl={6} >
    ...
    render() {
      ...
      return (
        <PageHeaderWrapper title="权限列表">
          ...
          <CreateForm {...parentMethods} modalVisible={modalVisible} current={current} permitgroup={permitgroup} />
        </PageHeaderWrapper>
      );
    }
  }

  export default Form.create()(PermitsList);
